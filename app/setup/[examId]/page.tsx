...
export default function SetupPage() {
  ...
  useEffect(() => {
    ...
    const savedSetup = localStorage.getItem(`examSetup_${examId}`)
    if (savedSetup) {
  ...
  const saveSetup = () => {
    ...
    const finalSetup = {
      ...
      createdAt: examSetup.createdAt || Date.now(),
    }

    // Save to localStorage
    localStorage.setItem(`examSetup_${examId}`, JSON.stringify(finalSetup))

    // Update exams list
    const examsList = localStorage.getItem("examsList")
    let exams = examsList ? JSON.parse(examsList) : []

    if (isNewExam) {
      // Add to exams list if new
      exams.push(finalSetup)
    } else {
      // Update existing exam in the list
      exams = exams.map((exam: ExamSetup) => (exam.id === examId ? finalSetup : exam))
    }
    ...
    toast({
      title: "Success",
      description: `Exam ${isNewExam ? "created" : "updated"} successfully!`,
    })
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 className="text-3xl font-bold">{isNewExam ? "Create New Exam" : "Edit Exam"}</h1>
          ...
          <Card className="border-2 shadow-sm">
            ...
            <CardFooter className="flex justify-between bg-primary/5 p-6">
              ...
              <Button onClick={saveSetup} size="lg">
                <Save className="mr-2 h-4 w-4" /> {isNewExam ? "Create Exam" : "Update Exam"}
              </Button>
            </CardFooter>
          </Card>
        </TabsContent>
